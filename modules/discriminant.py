...
def ejecutar_analisis_discriminante(df, columna_objetivo, variables_predictoras):
    ...
    Retorna un diccionario con los resultados, tablas y figuras de Plotly listos para renderizar.
    """
    ...
    try:
        ...
        # Estadísticas por grupo (Clasificación)
        ...
        if n_funciones >= 2 and len(variables_predictoras) >= 2:
            ...
            if len(df_scores) > LIMITE_PUNTOS:
                # Muestreo estratificado: sacamos una cantidad proporcional de cada grupo
                puntos_por_grupo = max(1, int(LIMITE_PUNTOS / len(clases)))
                df_plot = df_scores.groupby('Grupo', group_keys=False).apply(
                    lambda x: x.sample(min(len(x), puntos_por_grupo), random_state=42)
                )
                nota_titulo = f" (Muestra representativa de {len(df_plot)} puntos)"
            else:
                df_plot = df_scores
                nota_titulo = ""
            ...
            for i, grupo in enumerate(clases):
                # 1. Dibujamos los puntos usando el DataFrame recortado (df_plot)
                df_grupo_plot = df_plot[df_plot['Grupo'] == grupo]
                fig_scores.add_trace(go.Scattergl(x=df_grupo_plot['DF1'], y=df_grupo_plot['DF2'], mode='markers',
                                                name=str(grupo), marker=dict(color=colores[i % len(colores)], size=8, opacity=0.7)))
                
                # 2. Dibujamos los centroides usando el DataFrame COMPLETO (df_scores)
                df_grupo_completo = df_scores[df_scores['Grupo'] == grupo]
                fig_scores.add_trace(go.Scattergl(x=[df_grupo_completo['DF1'].mean()], y=[df_grupo_completo['DF2'].mean()],
                                                mode='markers', name=f'Centroide {grupo}',
                                                marker=dict(color=colores[i % len(colores)], size=15, symbol='x', 
                                                            line=dict(color='black', width=2)))) # Borde negro para que resalte
                                                            
            fig_scores.update_layout(title=f"Mapa de Funciones Discriminantes{nota_titulo}", height=600, width=700)
        ...
        return {
            ...
            "historial_grupos": historial_cambios,
